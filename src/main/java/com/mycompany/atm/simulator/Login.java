 ...
 */
package com.mycompany.atm.simulator;
...
public class Login extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae){
        try{
           ...
           //result set is a java object that stores the result of the query i.e the row that satisfies the condition of the query
           
           
           //sign in 
           if(ae.getSource()==b1){
               String q = "select * from login where cardno = '"+a+"' and pin = '"+b+"'";
               //jump from one column to another
               ResultSet rs = c1.s.executeQuery(q);
               if(rs.next()){
                   //new Transactions().setVisible(true);
                   setVisible(false);
               }else{
                   JOptionPane.showMessageDialog(null,"Incorrect card number of password");
